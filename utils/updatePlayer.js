...
const cardDeck = dataUtils.cardPossibilities;

function genCardNames(randNums){
    const cardNames = randNums.map(nums => cardDeck[nums - 1].cardName);
    return cardNames;
}
...
function updatePlayerObj(randNums, cardNames, player) {
    ...
    const cardValues = randNums.map(nums => cardDeck[nums - 1].cardValue)
